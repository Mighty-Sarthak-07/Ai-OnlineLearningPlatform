            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {isCompleted && <CheckCircle className='text-green-600 size-5 ml-2' />}
                </AccordionTrigger>
                <AccordionContent className='px-4 pb-4'>
                  {chapter?.courseData?.topics?.map((topic, topicIndex) => (
                    <div
                      key={topicIndex}
                      className={`rounded-md p-3 mb-2 transition duration-200 ${isCompleted ? 'bg-green-100 border border-green-400' : 'bg-muted/40 border'}`}
                    >
                      <h4 className='text-base font-medium text-gray-800'>{topic?.topic}</h4>
                      <p className='text-sm text-muted-foreground mt-1'>{topic?.topicDescription}</p>
                    </div>
                  ))}
                </AccordionContent>
              </AccordionItem>
