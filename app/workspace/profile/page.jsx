...
import {
  ...
  Clock, Star, Shield, CalendarDays, TrendingUp,
} from 'lucide-react'
...
function Profile() {
  ...
  const stats = [
    { icon: BookOpen,   label: 'Courses Enrolled', value: '3',    color: 'text-violet-600',  bg: 'bg-violet-50', border: 'border-violet-100' },
    { icon: Trophy,     label: 'Completed',         value: '5',    color: 'text-amber-500',   bg: 'bg-amber-50',  border: 'border-amber-100'  },
    { icon: Flame,      label: 'Day Streak',        value: '7',    color: 'text-orange-500',  bg: 'bg-orange-50', border: 'border-orange-100' },
    { icon: TrendingUp, label: 'XP Earned',         value: '1.2K', color: 'text-emerald-600', bg: 'bg-emerald-50',border: 'border-emerald-100'},
  ]

  const badges = [
    { emoji: '🚀', label: 'Fast Learner',    desc: 'Completed a course in under a week' },
    { emoji: '🔥', label: 'On Fire',         desc: '7-day learning streak' },
    { emoji: '🎓', label: 'Graduate',        desc: 'Completed 5 courses' },
    { emoji: '⭐', label: 'Top Rated',       desc: 'Rated 3 courses 5 stars' },
  ]
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        >
          {stats.map(({ icon: Icon, label, value, color, bg, border }) => (
            <motion.div
              key={label}
              whileHover={{ y: -2 }}
              className={`flex items-center gap-3 p-4 rounded-2xl border ${border} ${bg} bg-white shadow-sm`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Left: Clerk UserProfile (takes 2 cols) */}
          <motion.div
          ...
          <div className="flex flex-col gap-5">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {badges.map(({ emoji, label, desc }) => (
                  <motion.div
                    key={label}
                    whileHover={{ scale: 1.04 }}
                    className="flex flex-col items-center text-center p-3 rounded-2xl
                      bg-slate-50 border border-slate-200 hover:border-violet-200 hover:bg-violet-50 transition-all"
                  >
            ...
            >
              ...
              <div className="space-y-2">
                {[
                  ...
                  { icon: Clock,    label: 'Recent Activity',   href: '/workspace',             color: 'text-fuchsia-600', bg: 'bg-fuchsia-50' },
            ...
            {/* Learning activity heatmap-style */}
            <motion.div
              initial={{ opacity: 0, x: 16 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              ...
              <div className="flex gap-1.5 justify-between">
                {['M','T','W','T','F','S','S'].map((day, i) => {
                  const levels = [3, 2, 4, 1, 3, 0, 2]
                  const l = levels[i]
                  const colors = ['bg-slate-200', 'bg-violet-200', 'bg-violet-400', 'bg-violet-500', 'bg-violet-700']
                  return (
                    <div key={i} className="flex flex-col items-center gap-1.5">
                      <div className={`w-7 h-7 rounded-lg ${colors[l]} transition-all`} title={`${l} activities`} />
              ...
              <p className="text-xs text-slate-500 mt-3">
                🔥 <strong className="text-slate-700">7-day streak!</strong> Keep it up.
              </p>
            </motion.div>
